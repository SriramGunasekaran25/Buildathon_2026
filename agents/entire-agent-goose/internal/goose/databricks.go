...
func (a *Agent) annotateDatabricksExport(event *protocol.EventJSON, payload gooseHookPayload) {
...
func buildDatabricksCheckpoint(event *protocol.EventJSON, payload gooseHookPayload) (DatabricksCheckpoint, error) {
	checkpoint := DatabricksCheckpoint{
		...
		Prompt:              firstNonEmpty(event.Prompt, payload.Message),
		Summary:             "",
	...
	if err != nil {
		...
		return DatabricksCheckpoint{}, fmt.Errorf("read exported session: %w", err)
	}
	...
	checkpoint.TranscriptAvailable = true
	checkpoint.Summary = export.Name
	checkpoint.Model = firstNonEmpty(checkpoint.Model, export.ModelConfig.ModelName)
	checkpoint.ModifiedFiles = modifiedFilesFromExport(export, 0)
	if checkpoint.Prompt == "" {
		checkpoint.Prompt = latestPromptFromExport(export)
	}
	return checkpoint, nil
}

func latestPromptFromExport(export *gooseExport) string {
	if export == nil {
		return ""
	}
	for i := len(export.Conversation) - 1; i >= 0; i-- {
		msg := export.Conversation[i]
		if msg.Role != "user" {
			continue
		}
...
func (e *databricksExporter) executeStatement(ctx context.Context, statement string) error {
	...
	if readErr != nil {
		return errors.Join(fmt.Errorf("Databricks request failed with status %s", resp.Status), readErr)
	}
